...
const EditOutfit = () => {
    ...
    useEffect(() => {
        const fetchOutfit = async () => {
            try {
                const response = await axios.get(`http://localhost:9000/mycloset/outfits/${id}`);
                ...
                setFormData({
                    ...
                    ocassionType: outfit.ocassionType,
                });
                console.log(outfit.clothings);
                setSelectedClothings(outfit.Clothings);
                setLink(outfit.link);
            } catch (error) {
    ...
    const checkUpdateDuplicateOutfitCall = async (clothingIds, excludeOutfitId) => {
        try {
            const response = await axios.post('http://localhost:9000/mycloset/outfits/check-update-duplicate', { clothingIds, excludeOutfitId });
    ...
    const validateClothingSelection = async () => {
        ...
        const isDuplicate = await checkUpdateDuplicateOutfitCall(clothingIds, id); // Pass clothingIds and outfit id to exclude
        if (isDuplicate) {
        ...
        if (canvas) {
            canvas.toBlob(async (blob) => {
                try {
                    ...
                    await axios.put(`http://localhost:9000/mycloset/outfits/${id}`, outfitData);
                    toast.success('Outfit updated successfully!', {
                        onClose: () => navigate("/myoutfits") // Navigate after the toast message closes
                    });
                } catch (error) {
                ...
                <FormContainer>
                    <CForm onSubmit={handleSubmit}>
                        ...
                        <CRow className="mb-3">
                            ...
                            <CCol sm={10}>
                                <SelectField className="mb-3" name="ocassionType" value={formData.ocassionType} onChange={handleInputChange} hasError={!!formErrors.ocassionType}>
